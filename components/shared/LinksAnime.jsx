...
const LinksAnime = ({ animes }) => {
    ...
    console.log('ose',firsta)

    console.log(animes.links)
    useEffect(() => {
        setfirsta(animes.links)
    }, [])
                    ...
                    <div className='home-sv-b w-full'>
                        {firsta.map((link, index) => {
                            return (
                                <motion.div
                                    key={index}
                                    className='w-full flex bg-opacity-40 max-xl:flex-col justify-around p-5 gap-7 text-center items-center shadow bg-white '
                                    initial='initial'
